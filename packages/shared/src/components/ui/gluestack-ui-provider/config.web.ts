// Web-only config: same token values as config.ts but as a plain object
// instead of nativewind's vars() — vars() imports nativewind which pulls in
// react-native-css-interop → react-native-safe-area-context → native code
// that Turbopack can't bundle for web.
export const config = {
  light: {
